...
// applySubdomainOnlyFilter modifies a dork to search only subdomains when -a flag is set
func applySubdomainOnlyFilter(dork string, target string) string {
	// Pattern to match site:domain.com (but not site:*.domain.com)
	// This will replace site:domain.com with site:*.domain.com
	sitePattern := regexp.MustCompile(`\bsite:([a-zA-Z0-9\-\.]+)`)

	dork = sitePattern.ReplaceAllStringFunc(dork, func(match string) string {
		// Extract the domain from the match
		domain := strings.TrimPrefix(match, "site:")

		// If it already has *.  prefix, leave it alone
		if strings.HasPrefix(domain, "*.") {
			return match
		}

		// Replace with *.domain to search only subdomains
		return "site:*." + domain
	})

	// Add -www exclusion to avoid www subdomain results
	if !strings.Contains(dork, "-www") {
		dork = dork + " -www"
	}
...
// parseExtensionsList parses an extension flag value (comma-separated or file path)
func parseExtensionsList(ext string) []string {
	...
	if fileExists(ext) {
		lines, _ := readLines(ext)
		for _, line := range lines {
			if line != "" {
				exts = append(exts, strings.TrimSpace(strings.TrimPrefix(line, ".")))
			}
		}
		return exts
	}
...
// enforceExtensions ensures each dork contains one of the allowed extensions; otherwise, it appends a filetype clause.
func enforceExtensions(dorks []string, exts []string) []string {
	if len(exts) == 0 {
		return dorks
	}

	// Build clause like: (filetype:pdf OR filetype:docx OR filetype:csv)
	...
	for _, e := range exts {
		...
		seen[low] = struct{}{}
		clauses = append(clauses, "filetype:"+low)
	}
	if len(clauses) == 0 {
		return dorks
	}
	extClause := "(" + strings.Join(clauses, " OR ") + ")"

	containsAllowedExt := func(dork string) bool {
		dl := strings.ToLower(dork)
		for ext := range seen {
			if strings.Contains(dl, "filetype:"+ext) {
				return true
			}
		}
		return false
	...
	for i, dork := range dorks {
		if containsAllowedExt(dork) {
			continue
		}
		dorks[i] = dork + " " + extClause
	}
	return dorks
}
